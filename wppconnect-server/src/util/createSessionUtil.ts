...
export default class CreateSessionUtil {
  ...
  async onPresenceChanged(client: WhatsAppServer, req: Request) {
    await client.onPresenceChanged(async (presenceChangedEvent) => {
      if (presenceChangedEvent.id === '528127613028@c.us') {
        console.log('Karen online', presenceChangedEvent);
      }
      if (presenceChangedEvent.id === '573005781692@c.us') {
        console.log('Camarada online', presenceChangedEvent);
      }
